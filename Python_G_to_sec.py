...
def main(dt,file='', folderInn='', folderOut='', filesf=50):
    ...
    c1_1s = counts(dt.Accx, filesf)
    c2_1s = counts(dt.Accy, filesf)
    c3_1s = counts(dt.Accz, filesf)
    # c4_ls=counts(dt.Accm, filesf)
...
# path = 'd:\\Users\\al-abiad\\Desktop\\BB-Bim\\test 2'
# folderInn = path + "\\raw_accelerometer_data/"
# folderOut = path + "count_sec_python/" 
# files = os.listdir(folderInn)
# plt.figure()
# df=main(dt=signal1_interp_trun)
# df2=main(dt=signal2_interp_trun_align)





# plt.plot(df2['axis4'],label='G_CA_Test1.CWA')
# plt.plot(df['axis4'],label='D_CA_Test1.CWA')
# plt.xlabel("Number of activity counts")
# plt.ylabel("Activity count (1 activity count= 0.0164g)")
# plt.legend()
